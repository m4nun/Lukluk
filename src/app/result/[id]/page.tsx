...
import { ArrowLeft, RefreshCw, Check, Loader2, Shield, Brain, Sparkles } from "lucide-react";
...
export default function ResultPage() {
  ...
  const selectedPetId =
    typeof window !== "undefined"
      ? sessionStorage.getItem("lukluk_selected_pet") || matches[0]?.pet_type_profile_id
      : null;
  ...
  function selectPet(petTypeProfileId: string) {
    sessionStorage.setItem("lukluk_selected_pet", petTypeProfileId);
  }

  async function handleCreateWorkspace(petTypeProfileId: string) {
    selectPet(petTypeProfileId);
    setCreating(petTypeProfileId);
    setSubscribeError("");
    ...
    try {
      ...
      if (res.status === 401) {
        // Not logged in — send to auth, then back to this page
        router.push(`/auth/google?next=/result/${params.id}`);
        return;
      }

      if (res.status === 402) {
        // Need subscription
        setSubscribeError("subscription");
        setCreating(null);
        return;
      }
  ...
  async function handleSubscribe(petTypeProfileId: string) {
    selectPet(petTypeProfileId);
    setSubscribing(true);
    setSubscribeError("");
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        if (data.mode === "demo") {
          // Demo mode — auto-subscribe in mock, then refresh
          setSubscribing(false);
          handleCreateWorkspace(petTypeProfileId);
          return;
        }
        ...
        {!loading && !error && matches.length > 0 && (
          <>
            <div className="mt-8 space-y-5 pb-12">
              {matches.map((m, i) => {
                const logoSrc = getPetLogo(m.pet_type_profile_id);
                const isCreating = creating === m.pet_type_profile_id;
                return (
                  <div
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3">
                        <h3 className={`font-bold tracking-tight ${m.rank === 1 ? "text-2xl" : "text-xl"}`}>
                      ...
                      {/* Action button per match */}
                      <div className="mt-4">
                        <button
                          onClick={() => handleCreateWorkspace(m.pet_type_profile_id)}
                          disabled={isCreating || subscribing}
                          className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-2 text-sm font-semibold text-primary-foreground shadow-md transition-all hover:bg-primary/90 hover:-translate-y-0.5 disabled:opacity-60"
                        >
                          {isCreating ? (
                            <>
            ...
            {/* Subscription prompt when needed */}
            {subscribeError === "subscription" && (
              <div className="mb-12 rounded-2xl bg-primary/5 border border-primary/20 p-10 text-center">
                <h3 className="text-2xl font-bold tracking-tight">Unlock Full Access</h3>
                ...
                <button
                  onClick={() => handleSubscribe(selectedPetId || matches[0].pet_type_profile_id)}
