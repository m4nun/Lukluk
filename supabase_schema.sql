-- ============================================================
-- Lukluk v1 — Supabase Database Schema
-- ============================================================
-- Notes:
--   - Users table comes from Supabase Auth (auth.users)
--   - LangGraph PostgresSaver auto-creates: checkpoints, checkpoint_blobs, checkpoint_writes
--   - All timestamps are timestamptz (UTC)
--   - pet_type_profiles is seeded from YAML at deploy time (build step), not user-editable
--   - All tables have RLS enabled; policies defined per role
-- ============================================================
...
-- Created on auth.users insert via trigger.
-- ============================================================
...
-- 2. Pet Type Profiles
-- ============================================================
-- Global pet knowledge seeded from YAML at deploy time.
-- Readable by everyone. Writable by build step only.
-- ============================================================
CREATE TABLE public.pet_type_profiles (
  id              TEXT PRIMARY KEY,                      -- slug from YAML, e.g. "golden-retriever"
  name            TEXT NOT NULL,                         -- display name
  species         TEXT NOT NULL,                         -- "dog" | "cat" | "rabbit" | etc.
  breed_or_category TEXT NOT NULL,
  description     TEXT NOT NULL,
  mbti_label      TEXT NOT NULL,
  mbti_traits     JSONB NOT NULL DEFAULT '[]',           -- string[]
  mbti_description TEXT NOT NULL,

  -- Appearance
  size              TEXT NOT NULL,                       -- "small"|"medium"|"large"|"extra_large"
  ...
  budget_tier            TEXT NOT NULL,                  -- "low"|"medium"|"high"|"very_high"
  ...
  time_tier                TEXT NOT NULL,                -- "low"|"medium"|"high"

  -- Space
  minimum_sq_meters         REAL NOT NULL,
  space_tier                TEXT NOT NULL,               -- "apartment"|"house"|"large_house_outdoor"
  ...
  -- Allergies
  allergen_level  TEXT NOT NULL,                         -- "none"|"low"|"medium"|"high"
  hypoallergenic  BOOLEAN NOT NULL DEFAULT false,
  allergy_notes   TEXT,

  -- Noise
  noise_level             TEXT NOT NULL,                 -- "silent"|"quiet"|"moderate"|"vocal"|"very_vocal"
  ...
  restricted_regions       JSONB NOT NULL DEFAULT '[]',  -- string[]
  ...
  boarding_difficulty TEXT NOT NULL,                     -- "easy"|"moderate"|"hard"
  ...
  grooming_complexity    TEXT NOT NULL,                  -- "low"|"medium"|"high"
  feeding_per_day        INTEGER NOT NULL,
  special_diet_needs     BOOLEAN NOT NULL DEFAULT false,
  exercise_type          TEXT NOT NULL,
  common_health_issues   JSONB NOT NULL DEFAULT '[]',    -- string[]
  vet_visit_frequency    TEXT NOT NULL,

  -- Concerns (denormalized for matching)
  concerns              JSONB NOT NULL DEFAULT '[]',     -- [{id, title, severity, description}]

  -- Matching trait scores (1-10)
  ...
  -- Agent guidance (long-form text for system prompt)
...
-- ============================================================
-- Built from Fit Quiz answers. Saved once when user gets result.
-- Each user can have multiple (re-takes). Only latest per session.
-- ============================================================
CREATE TABLE public.lifestyle_profiles (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id         UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,

  -- Quiz fixed answers (10 dimensions, scored)
  ...
  --   "has_existing_pets": true,
  --   "existing_pet_types": ["cat"],
  --   "experience_level": "beginner",
  --   "child_in_household": false
  -- }

  -- LLM follow-up questions and answers (if any were asked)
...
-- 4. Match Results
-- ============================================================
-- One per quiz completion. Links lifestyle → pet matches.
-- ============================================================
CREATE TABLE public.match_results (
  ...
  -- Top 3 matches (ordered by responsible fit, then preference)
  top_matches         JSONB NOT NULL DEFAULT '[]',
  -- Example: [
  --   {
  --     "pet_type_profile_id": "golden-retriever",
  --     "rank": 1,
  --     "responsible_fit_score": 92,
  --     "mbti_match_score": 85,
  --     "explanation": "Great budget fit, high time match, no allergy issues"
  --   },
  --   ...
  -- ]
...
-- ============================================================
-- User workspace for exploring a pet type BEFORE ownership.
-- One per pet type per user (can plan multiple simultaneously).
-- ============================================================
CREATE TABLE public.planning_pet_profiles (
  id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id             UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  ...
  -- Planning info (user-editable and agent-editable)
  planning_name       TEXT,                               -- e.g. "Future Corgi"
  decision_status     TEXT NOT NULL DEFAULT 'exploring',  -- "exploring"|"considering"|"ready_to_buy"|"not_a_fit"|"already_have"

  -- Left panel: estimated expenses (denormalized for snapshot)
  estimated_expenses  JSONB NOT NULL DEFAULT '[]',
  -- Example: [
  --   { "category": "initial_cost", "item": "adoption_fee", "amount_thb": 5000, "note": "..." },
  --   ...
  -- ]

  -- Left panel: concern checklist (denormalized snapshot)
  concern_checklist   JSONB NOT NULL DEFAULT '[]',
  -- Example: [
  --   { "concern_id": "heavy_shedding", "status": "unresolved", "note": "..." },
  --   ...
  -- ]

  -- Ownership transition
  has_ownership        BOOLEAN NOT NULL DEFAULT false,
  owned_pet_profile_id  UUID,                             -- filled when user marks ownership
...
-- ============================================================
-- Converted from Planning Pet Profile when user marks "I already have this pet."
-- ============================================================
CREATE TABLE public.owned_pet_profiles (
  id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id             UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  planning_pet_profile_id UUID NOT NULL REFERENCES public.planning_pet_profiles(id) ON DELETE RESTRICT,
  pet_type_profile_id  TEXT NOT NULL REFERENCES public.pet_type_profiles(id) ON DELETE RESTRICT,

  -- Required ownership setup
  pet_name            TEXT NOT NULL,
  age_life_stage      TEXT NOT NULL,                       -- "puppy/kitten"|"young_adult"|"adult"|"senior"
  got_date            DATE,                                -- NULL = "I don't remember"
  got_date_unknown    BOOLEAN NOT NULL DEFAULT false,

  -- Optional ownership details
  weight_kg           REAL,
  ...
  -- Left panel: actual expenses
  actual_expenses     JSONB NOT NULL DEFAULT '[]',

  -- Left panel: activity schedule
...
-- 7. Agent Drafts
-- ============================================================
-- Pending agent-generated edits awaiting user confirmation.
-- ============================================================
CREATE TABLE public.agent_drafts (
  ...
  -- What the draft touches
  target              TEXT NOT NULL,                       -- "estimated_expenses" | "concern_checklist" | "decision_status"
  field_path          TEXT,                                -- dot notation for specific field (null = whole target)

  -- The draft content
  current_value       JSONB,                               -- value BEFORE the draft (for rollback reference)
  proposed_value      JSONB NOT NULL,                      -- the new proposed value

  -- Status
  status              TEXT NOT NULL DEFAULT 'pending',     -- "pending" | "confirmed" | "rejected" | "stale"
...
-- ============================================================
-- 8. Agent Sessions (thread mapping for LangGraph)
-- ============================================================
-- Maps a planning pet profile to a LangGraph thread_id for checkpointing.
-- =
-- ============================================================
CREATE TABLE public.agent_threads (
  ...
  -- Metadata
  agent_type            TEXT NOT NULL DEFAULT 'decision',   -- "decision" | "care"
  is_active             BOOLEAN NOT NULL DEFAULT true,
  created_at            TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at            TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- 9. Owner Experiences
-- ============================================================
-- User-submitted anecdotes from self-declared owners.
-- Go live immediately (v1: no moderation queue).
-- ============================================================
CREATE TABLE public.owner_experiences (
  id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id             UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  ...
  ownership_duration   TEXT,                                -- e.g. "2 years", "6 months"
  pet_name_or_number   TEXT,                                -- how many they've had of this type

  -- Content
  title               TEXT NOT NULL,
  body                TEXT NOT NULL,

  -- Safety (v1: simple flags, no pre-moderation)
...
-- 10. Subscriptions
-- ============================================================
-- Stripe subscription tracking. Test mode only for v1.
-- ============================================================
...
-- ============================================================
-- 11. Estimated Expenses (normalized, agent-editable rows)
-- ============================================================
-- Each row = one expense item in the left panel expense table.
-- Mirrors the denormalized JSONB in planning_pet_profiles.estimated_expenses.
-- We keep both: JSONB for snapshot/queries, this for agent tool calls.
-- ============================================================
CREATE TABLE public.estimated_expenses (
  id                    UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  planning_pet_profile_id UUID NOT NULL REFERENCES public.planning_pet_profiles(id) ON DELETE CASCADE,
  category              TEXT NOT NULL,                     -- "initial" | "monthly" | "annual" | "one_time"
  item                  TEXT NOT NULL,                     -- e.g. "adoption_fee", "food", "vet_visit"
...
-- ============================================================
-- Mirrors planning_pet_profiles.concern_checklist JSONB.
-- ============================================================
CREATE TABLE public.concern_checklist_items (
  id                    UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  planning_pet_profile_id UUID NOT NULL REFERENCES public.planning_pet_profiles(id) ON DELETE CASCADE,
  concern_id            TEXT NOT NULL,                     -- matches pet_type_profiles.concerns[].id
  title                 TEXT NOT NULL,
  status                TEXT NOT NULL DEFAULT 'unresolved', -- "unresolved" | "resolved" | "not_applicable"
  note                  TEXT,
  sort_order            SMALLINT NOT NULL DEFAULT 0,
  created_at            TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at            TIMESTAMPTZ NOT NULL DEFAULT now()
...
CREATE INDEX idx_pet_type_profiles_species ON public.pet_type_profiles(species);
CREATE INDEX idx_pet_type_profiles_budget_tier ON public.pet_type_profiles(budget_tier);
CREATE INDEX idx_pet_type_profiles_experience_level ON public.pet_type_profiles(experience_level);
CREATE INDEX idx_lifestyle_profiles_user_id ON public.lifestyle_profiles(user_id);
CREATE INDEX idx_lifestyle_profiles_is_latest ON public.lifestyle_profiles(user_id, is_latest);
CREATE INDEX idx_match_results_user_id ON public.match_results(user_id);
CREATE INDEX idx_match_results_lifestyle ON public.match_results(lifestyle_profile_id);
CREATE INDEX idx_planning_pet_profiles_user ON public.planning_pet_profiles(user_id);
CREATE INDEX idx_planning_pet_profiles_type ON public.planning_pet_profiles(pet_type_profile_id);
CREATE INDEX idx_owned_pet_profiles_user ON public.owned_pet_profiles(user_id);
CREATE INDEX idx_agent_drafts_profile ON public.agent_drafts(planning_pet_profile_id);
CREATE INDEX idx_agent_drafts_status ON public.agent_drafts(planning_pet_profile_id, status);
CREATE INDEX idx_agent_threads_planning ON public.agent_threads(planning_pet_profile_id);
CREATE INDEX idx_agent_threads_owned ON public.agent_threads(owned_pet_profile_id);
CREATE INDEX idx_owner_experiences_pet ON public.owner_experiences(pet_type_profile_id);
CREATE INDEX idx_owner_experiences_user ON public.owner_experiences(user_id);
CREATE INDEX idx_estimated_expenses_profile ON public.estimated_expenses(planning_pet_profile_id);
CREATE INDEX idx_concern_checklist_profile ON public.concern_checklist_items(planning_pet_profile_id);
...
-- Pet Type Profiles: everyone reads, service_role writes (build step)
...
CREATE OR REPLACE TRIGGER on_lifestyle_profile_created
